...
export const getTaskComments = async (taskId: number): Promise<Comment[]> => {
  const response = await taskManagerApi.api.get<Comment[]>(`/tasks/${taskId}/comments`);
  return response.data;
};

export const addCommentToTask = async (taskId: number, content: string): Promise<Comment> => {
  const response = await taskManagerApi.api.post<Comment>(`/tasks/${taskId}/comments`, content, {
